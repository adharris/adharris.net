...
module JekyllModels
  ...
  class ::Jekyll::ModelGenerator < ::Jekyll::Generator
    ...
    def write_model_index(site, model_name)
      ...
      when "pretty"
        index = ModelPage.new(site, site.config['source'], "", model_name, "_"+model_name, 'index.html', Time.parse("1900-01-01") )
      else
    ...
    def write_model_instance_indexes(site, model_name)
      ...
      models.each do |mdl|
        ...
        when "pretty"
          index = ModelPage.new(site, site.config['source'], model_name, mdl['mdl_name'], "_"+model_name, template_file, mdl['timestamp'])
        else
          index = ModelPage.new(site, site.config['source'], File.join(model_name, mdl['mdl_name']), 'index.html', "_"+model_name, template_file, mdl['timestamp'])
        end
